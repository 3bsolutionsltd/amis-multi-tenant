...
export function MarkDetailPage() {
  ...
  const updateRow = useCallback((key: number, field: "student_id" | "score", value: string) => {
    setDraftRows((rows) =>
  ...
  function handleSaveEntries() {
    setEntriesError(null);
    setEntriesSuccess(false);
    const valid = draftRows.filter((r) => r.student_id.trim() && r.score.trim());
    if (valid.length === 0) {
    ...
    if (invalid.length > 0) {
      setEntriesError(`Invalid score on row(s): ${invalid.map((r) => r.student_id).join(", ")}`);
      return;
    }
    entriesMut.mutate(valid.map((r) => ({ student_id: r.student_id.trim(), score: Number(r.score) })));
  }
    ...
    <div>
              ...
              <TD>
                <div>
                  {entry.first_name || entry.last_name
                    ? <span style={{ fontWeight: 500 }}>{entry.first_name} {entry.last_name}</span>
                    : null}
                  <span style={{ fontFamily: "monospace", fontSize: 11, color: C.gray400, display: "block" }}>
                    {entry.student_id}
                  </span>
                </div>
              </TD>
      ...
      {!isPublished && (
        <Card padding="20px 24px" style={{ marginBottom: 20 }}>
          ...
          {/* Column headers */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 110px 36px", gap: 8, marginBottom: 6 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: C.gray500, textTransform: "uppercase", letterSpacing: "0.05em" }}>Student ID</span>
            <span style={{ fontSize: 11, fontWeight: 700, color: C.gray500, textTransform: "uppercase", letterSpacing: "0.05em" }}>Score</span>
            <span />
          </div>

          {draftRows.map((row) => (
            <div key={row.key} style={{ display: "grid", gridTemplateColumns: "1fr 110px 36px", gap: 8, marginBottom: 8 }}>
              <input
                style={{ ...inputCss, fontFamily: "monospace", fontSize: 13 }}
                placeholder="Student UUID"
                value={row.student_id}
                onChange={(e) => updateRow(row.key, "student_id", e.target.value)}
              />
              <input
          ...
          {entriesSuccess && (
            <p style={{ color: "#16a34a", fontSize: 13, margin: "0 0 10px", fontWeight: 500 }}>
          ...
          >
            {entriesMut.isPending ? "Saving…" : `Save ${draftRows.filter((r) => r.student_id.trim()).length || ""} Entries`}
          </PrimaryBtn>
        </Card>
      )}
