...
export async function configRoutes(app: FastifyInstance) {
  ...
  // ------------------------------------------------------------------ POST /config/draft
  app.post("/config/draft", async (req, reply) => {
  ...
  app.post("/config/validate", async (req, reply) => {
    const tid = tenantHeader(req);
    if (!tid)
      return reply.status(400).send({ error: "x-tenant-id header required" });
  ...
  // ------------------------------------------------------------------ POST /config/publish
  app.post("/config/publish", async (req, reply) => {
    const tid = tenantHeader(req);
    if (!tid)
      return reply.status(400).send({ error: "x-tenant-id header required" });
  ...
  app.post("/config/rollback", async (req, reply) => {
    const tid = tenantHeader(req);
    if (!tid)
      return reply.status(400).send({ error: "x-tenant-id header required" });
